using static _2048.Program.Direction;
...
public class Program
{
    ...
    public static void Main(string[] args)
    {
...

        {
            ...
            while (true)
            {
                ...
                while (true)
                {
                    ...
                    bool IsNull((int X, int Y) point)
                    {
                        return board[point.X, point.Y] is null;
                    }

                    var nullCount = BoardValues(board).Count(IsNull);
                    if (nullCount is 0) goto GameOver;
                    var index = Random.Shared.Next(0, nullCount);
        ...
        bool TryUpdate(int?[,] board, ref int score, Direction direction)
        {
            ...
            var locked = new bool[board.GetLength(0), board.GetLength(1)];
            ...
            for (var j = 0; j < board.GetLength(1); j++)
            {
                ...
                if (board[adji, adjj] == board[tempi, tempj])
                {
                    board[adji, adjj] += board[tempi, tempj];
        ...
        {
            var horizontal = board.GetLength(0) * 8;
            ...
            for (var i = board.GetLength(1) - 1; i >= 0; i--)
            {
                Console.Write("║");
